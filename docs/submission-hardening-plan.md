...
> **Поправка по факту (PR 2, `90678380`).** Первый пункт проверки противоречил сам себе: при fail-closed дефолте (`() => false`) четыре существующих теста обязаны упасть, а не «остаться зелёными». Реализовано сформулированное намерение — четыре старых теста теперь явно передают `autoShowOn`, — а дефолт остался `false`, чтобы забытый аргумент не открывал окно.
...
> **Поправка по факту (PR 6, `da7849de`).** Формулировка «поле в проде всегда отсутствует» **неверна**, и её не стоит повторять. Overwolf регистрирует `gep_internal` **сам**: в логе GEP видно `onRequiredFeaturesChanged: ["game_info","match_info","gep_internal"]`, а в реальном диагностическом снимке он присутствовал — `GEP snapshot: game_info(steam_id) gep_internal(version_info)`. Фича к тому же документирована для Deadlock (страница supported-games, «Since GEP Ver. 263.0»; локальный GEP 312.5.2).
>
> Поэтому PR 6 — **не** починка пустого поля, а (1) явный запрос того, что приложение читает, вместо опоры на недокументированную авторегистрацию, (2) снятие ловушки с полным реджектом и (3) честная диагностика. Подробности — `roadmap.md`, раздел 4.2.
>
> Ещё поправка: поле ответа Overwolf — **`supportedFeatures`**, а не `features` (в `global.d.ts` была именно опечатка `features`, и она жила незамеченной, потому что поле никто не читал). Проверять `gep_internal` надо по странице supported-games, **а не по фиду статусов** `game-events-status.overwolf.com/<id>_prod.json`: фид перечисляет только игровые фичи и внутреннюю опускает.
...
> **Поправка по факту.** Урезать Settings не стали. Реальных пользователей у клиента пока нет — `overwolf-production-release.md` прямо говорит, что валидация идёт через Developer Mode, а сборка OPK и публикация вне текущего объёма, — поэтому «промежуточный прод» никому не вредит. А вырезание Overlay-группы оставило бы чекбокс авто-показа из PR 2 без единого места в UI, то есть настройка существовала бы и была недоступна.
...
> **Поправка по факту (PR 10, `eff08f40`).** Placeholder `api.invalid` **не вводился** — сборка теперь просто падает, если `OVERWOLF_API_BASE_URL` не задан. Это строго сильнее предложенного: сборка, которая не может завершиться, не может и уехать, поэтому placeholder не имеет ни одного шанса попасть в store-пакет. Случай «уехал placeholder» закрыт со стороны валидатора — он отвергает зарезервированные имена из RFC 2606/6761 (`.invalid`, `.example`, `.test`, `.localhost`), то есть ловит намерение, а не одну конкретную строку.
>
> **Риск из этого раздела снят структурно, а не дисциплиной.** «Менять оба значения в одном коммите» больше не требуется: константа скрипта переименована в `BUNDLE_SOURCE_API_BASE_URL`, и тест сверяет её с литералом из `src/index.ts`. Расхождение падает мгновенно и называет причину — раньше оно всплывало сообщением про отсутствующий в бандле URL, то есть указывало не туда.
>
> **Совет из сообщения об ошибке пришлось исправить по факту.** `http://localhost:3000` конфигуратор принимает, но `yarn build` включает store-ready-гейт, и валидатор такую сборку валит (`externally_connectable still contains a development origin`). Рабочий путь для локального origin — `build:bundle` + `configure:api`; проверено прогоном.
>
> **Оба CI-джоба, собирающие клиент, теперь передают origin явно** — иначе они бы просто упали. Значение публичное, оно и так лежит в репозитории.
