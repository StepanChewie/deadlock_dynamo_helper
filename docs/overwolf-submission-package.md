...
Скриншоты снимаются отдельно (раздел 4); ниже — текст. **Окон теперь два** — `dynamo_warning` удалён.
...
### Третьего окна больше нет

`dynamo_warning` **удалён** — окно было объявлено в манифесте, но его никто не открывал
(`src/index.ts` его не упоминает, и тест `statlocker-only-serving-cutover.spec.ts` это утверждает).
Владелец: «мы его щас не юзаем». Удалены: блок окна из манифеста, `dynamo_warning.html`,
`src/dynamo_warning.ts`, entry в webpack, проверка `dist/dynamo_warning.js` в валидаторе, а также
обновлены два спека. OPK стал 9 файлов вместо 11.

Побочно снялся дефект: попап был **полностью на русском** при английских остальных окнах и листинге.

### Рекламный контейнер

`#ow-ad-container` — в **левом сайдбаре окна desktop, внизу слева**, под навигацией, зарезервировано
250 px по высоте. Скрыт при ширине меньше 1280 px.

**Что писать в форму:** он **пустой в v1, рекламная сеть к нему не подключена** — приложение
бесплатное и без монетизации. Это тот же вопрос, что и в разделе 0, и формулировать его надо
честно, а не умалчивать: ревьюер всё равно откроет окно и увидит пустое место.
