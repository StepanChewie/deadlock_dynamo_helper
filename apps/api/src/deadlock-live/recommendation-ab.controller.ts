...
@Controller('deadlock-live/recommendation-ab/v1')
export class RecommendationAbController {
  constructor(private readonly reportService: RecommendationAbReportService) {}

  @Get('report')
  getReport(
    @Query('experimentId') experimentId: string,
    @Query('controlArm') controlArm: string,
    @Query('treatmentArm') treatmentArm: string,
