...
import {
  ...
  HeroBuildOfflineEvaluationRunState,
...
export const HERO_BUILD_OFFLINE_EVALUATION_LOW_MEMORY_MODE =
  'LOW_MEMORY_PER_HERO' as const;
export const HERO_BUILD_OFFLINE_EVALUATION_DEFAULT_BATCH_SIZE = 100;
export const HERO_BUILD_OFFLINE_EVALUATION_MIN_BATCH_SIZE = 10;
export const HERO_BUILD_OFFLINE_EVALUATION_MAX_BATCH_SIZE = 500;
export const HERO_BUILD_OFFLINE_EVALUATION_DEFAULT_MAX_RSS_MB = 1_200;
export const HERO_BUILD_OFFLINE_EVALUATION_MIN_MAX_RSS_MB = 256;
export const HERO_BUILD_OFFLINE_EVALUATION_MAX_MAX_RSS_MB = 16_384;

const BYTES_PER_MEGABYTE = 1024 * 1024;
const BATCH_SIZE_ENV = 'DEADLOCK_BUILD_EVALUATION_BATCH_SIZE';
const MAX_RSS_MB_ENV = 'DEADLOCK_BUILD_EVALUATION_MAX_RSS_MB';

export interface HeroBuildOfflineEvaluationLowMemoryStatus
  extends HeroBuildOfflineEvaluationStatus {
...
export interface HeroBuildOfflineEvaluationLowMemoryReport
  extends HeroBuildOfflineEvaluationReport {
  execution: {
    memoryMode: typeof HERO_BUILD_OFFLINE_EVALUATION_LOW_MEMORY_MODE;
    batchSize: number;
    maxRssMb: number;
...
@Injectable()
export class HeroBuildOfflineEvaluationLowMemoryService {
  private readonly logger = new Logger(
    HeroBuildOfflineEvaluationLowMemoryService.name,
  );
  ...
  private status: HeroBuildOfflineEvaluationLowMemoryStatus;
  ...
  start(
    ...
    this.runPromise = this.run(options)
      ...
      .catch((error) => {
        ...
        this.logger.error(`Low-memory offline build evaluation failed: ${message}`);
        throw error;
      })
      .finally(() => {
  ...
  getStatus(): HeroBuildOfflineEvaluationLowMemoryStatus {
    return {
      ...this.status,
  ...
  private async run(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `Using the existing recipe cache for low-memory offline evaluation: ${message}`,
      );
    }
    ...
    const heroIds = await this.collectHeroIds(split.selected);
    this.status = {
      ...this.status,
    ...
    const training = createEmptyTrainingSummary();
    const overall = new ComparisonAccumulator();
    const byHero: HeroBuildOfflineEvaluationHeroGroup[] = [];
    const byPhase = new Map<HeroBuildEvaluationPhase, ComparisonAccumulator>();
    const byHeroPhase: HeroBuildOfflineEvaluationHeroPhaseGroup[] = [];
    ...
    for (const [heroIndex, heroId] of heroIds.entries()) {
      this.status = {
        ...this.status,
        phase: 'TRAINING',
      ...
      for (const [batchIndex, descriptors] of chunkValues(
        ...
        const loaded = await this.loadHeroBatch(heroId, descriptors);
      ...
      addTrainingSummary(training, policySnapshot, matchupIndex);
      ...
      for (const [batchIndex, descriptors] of chunkValues(
        ...
        const loaded = await this.loadHeroBatch(heroId, descriptors);
        sourceTestPlayerCount += loaded.sourcePlayerCount;
      ...
      byHero.push({ heroId, comparison: heroAccumulator.build() });
    ...
    this.logger.log(
      `Low-memory evaluation processed ${report.test.evaluatedStepCount} held-out ` +
  ...
  private async collectHeroIds(
    ...
    for (const [index, batch] of batches.entries()) {
      const players = await this.matchPlayerRepository.find({
        where: { matchId: In(batch.map((descriptor) => descriptor.matchId)) },
      });
  ...
  private async loadHeroBatch(
    ...
    const requestedPlayers = await this.matchPlayerRepository.find({
    ...
    const roster = await this.matchPlayerRepository.find({
    ...
    for (const playerIdBatch of chunkValues(
      requestedPlayers.map((player) => Number(player.id)),
      this.batchSize * 4,
    )) {
      itemRows.push(
        ...(await this.matchPlayerItemRepository.find({
  ...
  private createIdleStatus(): HeroBuildOfflineEvaluationLowMemoryStatus {
    return {
      state: 'IDLE',
      totalMatchCount: 0,
      trainMatchCount: 0,
...
function createLowMemoryReport(input: {
  ...
  return {
    ...
    byPhase: (['EARLY', 'MID', 'LATE'] as HeroBuildEvaluationPhase[]).map(
      (phase): HeroBuildOfflineEvaluationPhaseGroup => ({
        phase,
        comparison:
          input.byPhase.get(phase)?.build() ?? new ComparisonAccumulator().build(),
      }),
    ),
    ...
    byOutcome: (['WIN', 'LOSS'] as HeroBuildEvaluationOutcome[]).map(
      (outcome): HeroBuildOfflineEvaluationOutcomeGroup => ({
        outcome,
        comparison:
          input.byOutcome.get(outcome)?.build() ??
          new ComparisonAccumulator().build(),
      }),
    ),
...
): HeroBuildOfflineEvaluationReport['split'] {
...
export function chunkValues<T>(values: readonly T[], batchSize: number): T[][] {
  const chunks: T[][] = [];
